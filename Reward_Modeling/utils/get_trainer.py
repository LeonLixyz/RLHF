...
def compute_loss_enn(model, p_1_ids, p_2_ids, p_1_att, p_2_att, z_samples, label):
    r_1 = model(input_ids = p_1_ids, attention_mask = p_1_att, z_samples = z_samples, return_dict = True).rewards
    r_2 = model(input_ids = p_2_ids, attention_mask = p_2_att, z_samples = z_samples, return_dict = True).rewards

    # reward diff
    r_diff = torch.where(label.unsqueeze(-1) == 1, r_1 - r_2, r_2 - r_1)
    loss = -torch.log(torch.sigmoid(r_diff))
    loss = loss.mean()
    
    return loss   

def compute_loss_vanilla(model, p_1_ids, p_2_ids, p_1_att, p_2_att, label):
...
def trainer(model, optimizer, scheduler, training_dataloader, eval_dataloader, joint_eval_dataloader, accelerator, logging_args, training_args, model_args):
    ...
    def print_metrics(accelerator, metrics, category="metric"):
        if category == "metric":
            ...
            accelerator.print(f'r_lose_average: {metrics["r_lose_average"]:.4f}, r_lose_min: {metrics["r_lose_min"]:.4f}, r_lose_max: {metrics["r_lose_max"]:.4f}, r_lose_std: {metrics["r_lose_std"]:.4f}')
            if logging_args.project == "reward-enn":
                ...
                accelerator.print(f'eta_lose_average: {metrics["eta_lose_average"]:.4f}, eta_lose_min: {metrics["eta_lose_min"]:.4f}, eta_lose_max: {metrics["eta_lose_max"]:.4f}, eta_lose_std: {metrics["eta_lose_std"]:.4f}')
                ...
                accelerator.print(f'p_lose_average: {metrics["p_lose_average"]:.4f}, p_lose_min: {metrics["p_lose_min"]:.4f}, p_lose_max: {metrics["p_lose_max"]:.4f}, p_lose_std: {metrics["p_lose_std"]:.4f}')
    ...
    def eval_logger(accelerator, metrics_storage, rewards_storage):
        if logging_args.project == "reward-enn":
            ...
            for eval_z_samples_size in eval_z_size_list:
                metrics, reward = distributed_evaluation(model, eval_dataloader, accelerator, model_args, logging_args, training_args, eval_z_samples_size)
                log_joint_likelihood, joint_log_likelihood = joint_distributed_eval(model, joint_eval_dataloader, training_args, logging_args, model_args, accelerator, eval_z_samples_size)
            ...
            metrics, reward = distributed_evaluation(model, eval_dataloader, accelerator, model_args, logging_args, training_args)
            log_joint_likelihood = joint_distributed_eval(model, joint_eval_dataloader, training_args, logging_args, model_args, accelerator)
            print_metrics(accelerator, metrics, "metric")
            accelerator.print('joint log likelihood:', log_joint_likelihood)
    ...
    for e in range(training_args.num_epochs):
        ...
        for i, batch in enumerate(training_dataloader):
            ...
            optimizer.zero_grad()
            p_1_ids, p_2_ids, p_1_att, p_2_att, label = batch
            ...
            if logging_args.project == "reward-enn":
                ...
                loss = compute_loss_enn(model = model, p_1_ids = p_1_ids, p_2_ids = p_2_ids, p_1_att = p_1_att, p_2_att = p_2_att, z_samples = current_z_samples, label = label)

            elif logging_args.project == "vanilla-reward":
                loss = compute_loss_vanilla(model = model, p_1_ids = p_1_ids, p_2_ids = p_2_ids, p_1_att = p_1_att, p_2_att = p_2_att, label = label)
